...
> {
  ...
  updateTimeSlider = (): void => {
    if (
      ...
      const songProgress =
        (this.state.currentTime / this.audioPlayer.current.duration) * 100;
  ...
  onTimeSliderChange = (event: SyntheticEvent): void => {
    if (this.progressBar.current && this.progressBar.current.value) {
      ...
      const timeFromProgressBar =
        (valueOfProgressBar * Number(this.state.duration)) / 100;
  ...
  whilePlaying = (): void => {
    if (this.progressBar.current && this.audioPlayer.current) {
      this.progressBar.current.value =
        this.audioPlayer.current.currentTime.toString();
    ...
    this.setState(
      ...
      () => {
        this.audioPlayer.current
          ...
          : console.log("ERROR: failed to pause song");
      }
    );
  };
    ...
    return (
      <div className="audioPlayer">
        ...
        <span className="audioPlayerUI">
          ...
          ></audio>
          <button className="forwardBackward" onClick={this.backThirty}>
            <BsArrowLeftCircle />
          </button>
          <button
            className="playPause"
          ...
          <button className="forwardBackward" onClick={this.forwardThirty}>
            <BsArrowRightCircle />
          </button>
        </span>
      </div>
